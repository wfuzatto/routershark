...
const API='/api';
function auth():Record<string,string>{const t=localStorage.getItem('routershark_token')||'';return t?{Authorization:`Bearer ${t}`}:{}}
function App(){
 const [packets,setPackets]=useState<Packet[]>([]),[selected,setSelected]=useState<Packet|null>(null),[decoded,setDecoded]=useState<unknown>(null),[hex,setHex]=useState(''),[paused,setPaused]=useState(false),[filter,setFilter]=useState(''),[search,setSearch]=useState(''),[status,setStatus]=useState('connecting');
